...
import numpy as np
import pandas as pd
from ndmo_red_node_max_fixed import (
    load_all_basins,
...
def denormalize_flow(solutions, mean, std):
    return solutions * std + mean

def batch_run():
    data_path = "dataset_utils/pyg_data_multi"
    raw_data_path = "rawdata/nodes_all.xlsx"
    save_dir = "results/PF_relaxed_max"
    ...
    fri_model_file = sorted([f for f in os.listdir(model_dir) if "GIN_FRI" in f],
                            key=lambda x: float(x.split("r2_")[-1].replace(".pth", "")), reverse=True)[0]
    dri_model_file = sorted([f for f in os.listdir(model_dir) if "GIN_DRI" in f],
                            key=lambda x: float(x.split("r2_")[-1].replace(".pth", "")), reverse=True)[0]
    ...
    for file in basin_files:
        ...
        try:
            ...
            original_FRI, original_DRI, flow_mean, flow_std = replace_flow_and_evaluate_multitask(
                data, node_flows, fri_model, dri_model)
            original_F = [original_FRI, original_DRI]

            population = generate_initial_population(data)
            if len(population) == 0:
                print("⚠️ No feasible solutions found for", basin_name)
                continue

            FRI_scores, DRI_scores, solutions = [], [], []
            for solution in population:
                data_temp = replace_pyg_flow_with_absolute(data.clone(), solution)
                FRI_pred, DRI_pred, _, _ = replace_flow_and_evaluate_multitask(
                    data_temp, solution, fri_model, dri_model)
                FRI_scores.append(FRI_pred)
                DRI_scores.append(DRI_pred)
            ...
            denorm_solutions = denormalize_flow(np.array(pareto_solutions), flow_mean, flow_std)

            save_pareto_solutions_with_scores(
                solutions=denorm_solutions,
                fri_scores=np.array(FRI_scores)[pareto_indices],
                dri_scores=np.array(DRI_scores)[pareto_indices],
                save_dir=save_dir,
